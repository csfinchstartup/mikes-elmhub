...
Elm.Json = Elm.Json || {};
Elm.Json.Decode = Elm.Json.Decode || {};
Elm.Json.Decode.Pipeline = Elm.Json.Decode.Pipeline || {};
Elm.Json.Decode.Pipeline.make = function (_elm) {
   "use strict";
   _elm.Json = _elm.Json || {};
   _elm.Json.Decode = _elm.Json.Decode || {};
   _elm.Json.Decode.Pipeline = _elm.Json.Decode.Pipeline || {};
   if (_elm.Json.Decode.Pipeline.values) return _elm.Json.Decode.Pipeline.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var decode = $Json$Decode.succeed;
   var resolveResult = function (resultDecoder) {
      return A2($Json$Decode.andThen,
      resultDecoder,
      function (result) {
         return A2($Json$Decode.customDecoder,$Json$Decode.succeed({ctor: "_Tuple0"}),function (_p0) {    return result;});
      });
   };
   var custom = F2(function (delegated,decoder) {
      return A2($Json$Decode.andThen,decoder,function (wrappedFn) {    return A2($Json$Decode.map,wrappedFn,delegated);});
   });
   var hardcoded = F2(function (val,decoder) {
      return A2($Json$Decode.andThen,decoder,function (wrappedFn) {    return A2($Json$Decode.map,wrappedFn,$Json$Decode.succeed(val));});
   });
   var optional = F4(function (key,valDecoder,fallback,decoder) {
      var maybeDecoder = A2($Json$Decode.andThen,
      $Json$Decode.maybe(A2($Json$Decode._op[":="],key,valDecoder)),
      function (_p1) {
         return $Json$Decode.succeed(A2($Maybe.withDefault,fallback,_p1));
      });
      return A2(custom,maybeDecoder,decoder);
   });
   var required = F3(function (key,valDecoder,decoder) {    return A2(custom,A2($Json$Decode._op[":="],key,valDecoder),decoder);});
   return _elm.Json.Decode.Pipeline.values = {_op: _op
                                             ,required: required
                                             ,optional: optional
...
Elm.Shims = Elm.Shims || {};
Elm.Shims.make = function (_elm) {
   "use strict";
   _elm.Shims = _elm.Shims || {};
   if (_elm.Shims.values) return _elm.Shims.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   return _elm.Shims.values = {_op: _op,performAction: performAction,onInput: onInput,onChange: onChange};
};
Elm.ElmHub = Elm.ElmHub || {};
Elm.ElmHub.make = function (_elm) {
   "use strict";
   _elm.ElmHub = _elm.ElmHub || {};
   if (_elm.ElmHub.values) return _elm.ElmHub.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var viewError = function (error) {
      var _p0 = error;
      if (_p0.ctor === "Just") {
            return A2($Html.span,_U.list([$Html$Attributes.$class("error")]),_U.list([$Html.text(_p0._0)]));
         } else {
            return $Html.text("");
         }
   };
   var viewResult = function (results) {
      var description = $String.isEmpty(results.description) ? $Html.text("") : A2($Html.small,
      _U.list([]),
      _U.list([A2($Html.p,_U.list([$Html$Attributes.$class("repo-description")]),_U.list([$Html.text(results.description)]))]));
      var privateMarker = results.is_private ? A2($Html.small,
      _U.list([]),
      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("is-private")]),_U.list([$Html.text("private")]))])) : $Html.text("");
      var nameTitle = A2($Html.span,_U.list([$Html$Attributes.$class("repo-name-title")]),_U.list([$Html.text(results.name)]));
      var seperator = A2($Html.span,_U.list([]),_U.list([$Html.text(" / ")]));
      var namespaceTitle = A2($Html.span,_U.list([$Html$Attributes.$class("repo-namespace-title")]),_U.list([$Html.text(results.namespace)]));
      return A2($Html.li,
      _U.list([$Html$Attributes.$class("repo-item")]),
      _U.list([A2($Html.a,_U.list([$Html$Attributes.href("#")]),_U.list([namespaceTitle,seperator,nameTitle,privateMarker,description]))
              ,A2($Html.hr,_U.list([]),_U.list([]))]));
   };
   var viewResults = F2(function (address,_p1) {
      var _p2 = _p1;
      var _p4 = _p2.reposPerPage;
      var _p3 = _p2.filterStr;
      return A2($Html.ul,
      ...
      A2($List.drop,
      (_p2.pageNum - 1) * _p4,
      A2($List.filter,function (r) {    return A2($String.startsWith,_p3,r.namespace) || _U.eq($String.isEmpty(_p3),true);},_p2.results)))));
   });
   ...
   var filteredLength = function (model) {
      return $String.isEmpty(model.filterStr) ? $List.length(model.results) : $List.length(A2($List.filter,
      function (r) {
   ...
   var viewFilter = F2(function (address,model) {
      ...
      _U.list([A2($Html.label,_U.list([$Html$Attributes.$for("namespace-filter")]),_U.list([$Html.text("Filter by ")]))
              ,A2($Html.select,
              _U.list([$Html$Attributes.$class("namespace-filter"),$Html$Attributes.name("namespace-filter"),A2($Shims.onChange,address,FilterByName)]),
              A2($List._op["::"],
              A2($Html.option,_U.list([$Html$Attributes.value(""),$Html$Attributes.selected(true)]),_U.list([$Html.text("All Accounts")])),
              A2($List.map,viewFilterOption,$Set.toList($Set.fromList(A2($List.map,function (_) {    return _.namespace;},model.results))))))
              ,A2($Html.hr,_U.list([]),_U.list([]))]));
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2(viewHeader,address,model),A2(viewResults,address,model),A2(viewPaginator,address,model)]));
   });
   var initialModel = {userName: "library",filterStr: "",pageNum: 1,results: _U.list([]),error: $Maybe.Nothing,reposPerPage: 10};
   ...
   var repoResultDecoder = A3($Json$Decode$Pipeline.required,
   "last_updated",
   $Json$Decode.string,
   ...
   var resultsDecoder = A2($Json$Decode._op[":="],"results",$Json$Decode.list(repoResultDecoder));
   var repoFeed = function (userName) {
      var url = A2($Basics._op["++"],"http://127.0.0.1:3000/api/repos/",userName);
      var task = A3($Shims.performAction,HandleRepoResult,HandleRepoError,A2($Http.get,resultsDecoder,url));
      return $Effects.task(task);
   };
   var update = F2(function (action,model) {
      var _p6 = action;
      switch (_p6.ctor)
      {case "JumpToPage": return {ctor: "_Tuple2",_0: _U.update(model,{pageNum: _p6._0}),_1: $Effects.none};
         case "SetUserName": return {ctor: "_Tuple2",_0: _U.update(model,{userName: _p6._0}),_1: $Effects.none};
         case "HandleRepoError": var _p7 = _p6._0;
           if (_p7.ctor === "UnexpectedPayload") {
                 return {ctor: "_Tuple2",_0: _U.update(model,{error: $Maybe.Just(_p7._0)}),_1: $Effects.none};
              } else {
                 return {ctor: "_Tuple2"
                        ,_0: _U.update(model,{error: $Maybe.Just("There was an error communicating with Elm Hub. Take a look at your username...")})
                        ,_1: $Effects.none};
              }
         case "HandleRepoResult": return {ctor: "_Tuple2",_0: _U.update(model,{results: _p6._0,error: $Maybe.Nothing}),_1: $Effects.none};
         case "Search": return {ctor: "_Tuple2",_0: _U.update(model,{results: _U.list([])}),_1: repoFeed(model.userName)};
         case "FilterByName": return {ctor: "_Tuple2",_0: _U.update(model,{filterStr: _p6._0,pageNum: 1}),_1: $Effects.none};
   ...
   var decodeResults = function (json) {
      var _p8 = A2($Json$Decode.decodeString,resultsDecoder,json);
      if (_p8.ctor === "Ok") {
            return _p8._0;
         } else {
            return _U.list([]);
   ...
   var Model = F6(function (a,b,c,d,e,f) {    return {userName: a,filterStr: b,pageNum: c,results: d,error: e,reposPerPage: f};});
   return _elm.ElmHub.values = {_op: _op
                               ...
                               ,repoResultDecoder: repoResultDecoder
                               ,FilterByName: FilterByName
                               ,PreviousPage: PreviousPage
                               ,NextPage: NextPage
                               ,HandleRepoResult: HandleRepoResult
...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({view: $ElmHub.view
                             ,update: $ElmHub.update
                             ,init: {ctor: "_Tuple2",_0: $ElmHub.initialModel,_1: $ElmHub.repoFeed($ElmHub.initialModel.userName)}
                             ,inputs: _U.list([])});
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var main = app.html;
   return _elm.Main.values = {_op: _op,main: main,app: app};
};
